...
namespace WorkItemManagementConsoleApp.Core.Contracts
{
    public interface IFactory
    {
        ...
        IBug CreateBug(string id, string title, string description, string priority,
            string severity, string status, Member assignee, List<string> steps);
        IStory CreateStory(string id, string title, string description, Member assignee,
            string priority, string storyStatus, string size);
